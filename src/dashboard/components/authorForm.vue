<template>
  <b-modal :id="id">
    <template #modal-header="{ close }">
      <h5>
        {{ modeEdit ? "Edit" : "Create" }} Author {{ data.id ? `#${data.id}` : null }}
      </h5>
    ...
    <template #default>
      ...
      <bookComponent
        ...
        :currentAuthor="data"
        @createSucceed="addBook"
        @deleteSucceed="subtractBook"
      ></bookComponent>
    </template>
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.modeEdit) {
        crud
          .update(this.$axios, this.root, tempdata.id, {
            id: tempdata.id,
            name: tempdata.name,
          })
          .then((res) => {
        ...
        crud
          .create(this.$axios, this.root, tempdata)
          .then((res) => {
    ...
    addBook() {
      this.data.book_count = this.data.book_count + 1;
    },
    subtractBook() {
      this.data.book_count = this.data.book_count - 1;
    },
