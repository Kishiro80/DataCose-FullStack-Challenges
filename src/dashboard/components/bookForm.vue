<template>
    <b-modal :id="id">
        <template #modal-header="{ close }">
            ...
            <h5>{{ modeEdit ? 'Edit' : 'Create' }} Book {{ data.id ? `#${data.id}` : null }}</h5>
            <b-button size="sm" variant="outline-danger" @click="close()"> X </b-button>
        </template>

        <template #default>
            <b-form @submit="save">
                <b-form-group id="input-group-1" label="Book Title:" label-for="input-1" description="Book Title?">
                    <b-form-input
                        id="input-1"
                        name="input-1"
                        v-model="data.title"
...
export default {
    ...
    methods: {
        ...
        async save() {
            ...
            if (this.modeEdit) {
                crud.update(this.$axios, this.root, this.data.id, cleandata)
                    .then((res) => {
                        res.author = author;
                ...
                crud.create(this.$axios, this.root, cleandata)
                    .then((res) => {
                        res.author = author;
